...
double tmcmc(int nsamps,
  ...
  */
  ...
  genPriorSamps(generator, u_distribution, n_distribution,
    spls, nsamps, ndim, alphas, betas, types);

  spls_flat = flatten(spls);

  for (int i = 0; i < nsamps; i++) {
    llik[i] = logLikelihood->eval(spls[i]);
    lprior[i] = LogPrior::eval(spls[i],alphas,betas,types);
  }
  ...
  do { // Start algorithm
    ...
    for (int isbSteps=0; isbSteps < nSteps; isbSteps++ ) {
      ...
      for (int i = 0; i < nsamps; i++) {
        ...
        lprior[i] = LogPrior::eval(phi,alphas,betas,types);
      }
      ...
      if (pid == 0){
        ...
        for (int ispl=0; ispl<nsplSt; ispl++) {
          ...
          AcceptRatio = beta * (all_llik[icomp] - llikSt[ispl])
                        + (all_lprior[icomp] - lpriorSt[ispl]);
    ...
    for (int j = 0; j < ndim; j++) {
      switch (types[j]){
        ...
        case 'G':
          ...
          while(spls[i][j] > boundary[j*2+1] || spls[i][j] < boundary[j*2])
            spls[i][j] =  alphas[j] + betas[j]*n_distribution(generator);
          break;
      }
    }
