...
int main (int argc, char *argv[]) {
  ...
  std::string proposalTypesFile = "proposal_types.dat";
  // Proposal hyperparameters, 2-column format, one line per parameter,
  // lower and upper bounds for uniform prior PDF,
  // mean and standard deviation for Gaussian prior PDF
  std::string proposalParamsFile = "proposal_params.dat";
  std::string proposalMeanFile = "proposal_mean.dat";
  std::string proposalInvCovFile = "proposal_cov_inv.dat";
  std::string proposalCovSqrtFile = "proposal_cov_sqrt.dat";
  ...
  logevid = tmcmc(nsamps, ran_seed, chainDim,
				  ...
				  proposalParamsFile, proposalTypesFile, &logLikelihood);
  */
  logevid = tmcmc(nsamps,ran_seed, chainDim, tmcmc_cv,
          write_flag, pid, np,
          priorParamsFile, priorTypesFile,
          proposalMeanFile,proposalInvCovFile,
          proposalCovSqrtFile, &logLikelihood);
